...
export const SunsetApi = ({city, latitude, longitude, timezone}) => {
    ...
    useEffect(() => {
        ...
        fetch(`https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&hourly=cloud_cover&daily=sunshine_duration`)
        .then(res => res.json())
        .then(json => {
    ...
    function formatTime(timeString) {
        ...
        if (typeof(timeString) ==  "string"){
            ...
            const sunsetLocal = new Date(sunsetUTC.getTime() + (timezone * 60 * 60 * 1000));
  ...
  return (
    ...
    <div className="weather-forecast">
            {dataArray.map((data, index) => (
                <div key={index}>
                    <p>{data.time}</p>
                    <img src="sun.svg"  style={{ height: data.sunshine / 1000 + 'px', width: data.sunshine / 1000 + 'px' }} />
                </div>
            ))}
        </div>
    </>
